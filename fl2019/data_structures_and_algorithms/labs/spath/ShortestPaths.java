...
public class ShortestPaths {
    ...
    public void run() {
    	...
    	//stuff i added
    	while (!pq.isEmpty()) {
    		VertexAndDist v1 = pq.extractMin();
    		
    		for (Edge e : v1.vertex.edgesFrom()) { // for each edge from vertex v1
    			Decreaser<VertexAndDist> decreaser = handles.get(e.to); //get decreaser at u; end of edge
    			VertexAndDist v2 = decreaser.getValue(); //v2 = vertex and distance from decreaser
    			if (weights.get(e) + v1.distance < v2.distance) { //if edge weight plus v1's dist < v2 (from decreaser)
    				VertexAndDist v3 = new VertexAndDist(v2.vertex, (weights.get(e) + v1.distance)); //new vertexanddist with new distance
    				decreaser.decrease(v3); //decrease to new v3
    				parentEdges.put(v2.vertex, e); //record parent edges of each vertex in parentEdges map
    			}
    		}
    	}
    }
    ...
    public LinkedList<Edge> returnPath(Vertex endVertex) {
    	...
    	//
    	//stuff i added
    	while (endVertex != startVertex) { //as long as the current ending vertex is not the starting vertex of the whole thing
    		Edge currentLast = parentEdges.get(endVertex); //current last edge is retrieved from map with endVertex key
    		path.addFirst(currentLast); //add currently last edge to path
    		endVertex = currentLast.from; //end vertex is updated to the end of the next edge, where currently last edge came from
    	}
    	return path;
    }
