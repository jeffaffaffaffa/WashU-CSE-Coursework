...
public class StringTable {
	...
	public StringTable(int nBuckets) {
    	...
    	//added a for loop to make list of records for each bucket in string table
    ...
    {  
    	...
    	//check if keys in bucket equal key of r
    	for (int i = 0; i < buckets[index].size(); ++i) {
    		if ((buckets[index].get(i).key).equals(key)) {
    			return false;
    		}
    	}
    	//increase size and add r to bucket if not already in bucket
    	size++;
    	buckets[index].add(r);
    	return true;
    }
    ...
    public Record find(String key) {
    	// TODO - implement this method
    	int index = toIndex(stringToHashCode(key));
    	//checks if key is in bucket and retrieves corresponding record
    	for (int i = 0; i < buckets[index].size(); ++i) {
    		if ((buckets[index].get(i).key).equals(key)) {
    ...
    public void remove(String key) {
    	// TODO - implement this method
    	int index = toIndex(stringToHashCode(key));
    	//checks if key is in bucket and removes corresponding record
    	for (int i = 0; i < buckets[index].size(); ++i) {
    		if ((buckets[index].get(i).key).equals(key)) {
    ...
    private int toIndex(int hashcode){
    	...
    	//knuth's 
    	double knuth = (Math.sqrt(5) - 1.0)/2.0;
  
    	return (int)(Math.abs((hashcode*knuth)%1.0)*nBuckets);
    }
